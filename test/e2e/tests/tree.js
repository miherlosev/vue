...
test('tree', async t => {
    await t.expect(treePage.allItems.count).eql(12)
    ...
    // expand root
    await t.click(treePage.topLevelRoot.name)
    ...
    // // add items to root
    await t.click(treePage.topLevelRoot.addNew)
    ...
    await t.expect(secondLevelRoot.childNodesContainer.visible).ok()
    ...
    await t.doubleClick(treePage.topLevelRoot.childNodesContainer.child(0))
    ...
    await t.expect(helloTreeItem.name.textContent).contains('[-]')
